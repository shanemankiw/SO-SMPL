...
class RandomCameraDataModuleConfig:
    ...
    eval_up_bias: float = 0.0
    light_sample_strategy: str = "dreamfusion"
    batch_uniform_azimuth: bool = True
    proba_front: float = 0.24  # 0.24
    proba_side: float = 0.08  # 0.08
    proba_back: float = 0.24  # 0.24
    # from up looking down
    proba_up: float = 0.0
...
class RandomCameraIterableDataset(IterableDataset):
    ...
    def _gen_face_cameras(self):
        ...
        camera_distances = (
            torch.ones_like(elevation_deg) * 1.0
    ...
    def _gen_body_cameras(self):
        ...
        p = random.random()
        if p < 0.8 and self.cfg.rand_ele:
            range_ele = 15.0
            elevation_deg = torch.rand(self.cfg.batch_size) * (
                (15.0 + range_ele) - (15.0 - range_ele)
            ) + (
                15.0 - range_ele
            )  # [0, 30]
            camera_distances = (
                torch.rand(self.cfg.batch_size) * (2.0 - (1.0)) + 1.0
            )  # [1.0, 2.0]
        else:
            elevation_deg = (
                torch.rand(self.cfg.batch_size)
        ...
        p = random.random()
        if p < self.cfg.proba_front:
            azimuth_deg = torch.rand(self.cfg.batch_size) * (80.0 - (-80.0)) - 80.0
        elif p < (self.cfg.proba_front + self.cfg.proba_side / 2.0):
            range_azi = 60.0
            azimuth_deg = torch.rand(self.cfg.batch_size) * (
                (90.0 + range_azi) - (90.0 - range_azi)
            ) + (90.0 - range_azi)
        elif p < (self.cfg.proba_front + self.cfg.proba_side):
            range_azi = 60.0
            azimuth_deg = torch.rand(self.cfg.batch_size) * (
                (270.0 + range_azi) - (270.0 - range_azi)
            ) + (270.0 - range_azi)
        elif p < (self.cfg.proba_front + self.cfg.proba_side + self.cfg.proba_back):
            range_azi = 60.0
            azimuth_deg = torch.rand(self.cfg.batch_size) * (
                (180.0 + range_azi) - (180.0 - range_azi)
            ) + (180.0 - range_azi)
        else:
            azimuth_deg = (
        ...
        # NOTE(wjh)
        # for clothes, camera views should adjust to a center that is little higher
        # up_margin = random.random() * (0.25 - 0.1) + 0.1 # [0.15, 0.3]

        center = center + up * self.cfg.up_bias  # 0.3 is the range of up
        ...
        """control_signals = []
        for mvp in mvp_mtx:
            control_image = draw_body(self.body_joints, mvp[None, :], [512, 512])
            # DEBUG
            import cv2
            cv2.imwrite('checkouts/control_image.png', control_image)
...
class RandomCameraDataset(Dataset):
    def __init__(self, cfg: Any, split: str) -> None:
        ...
        center = center + self.cfg.eval_up_bias * up
...
class GenImageSequenceDataset(Dataset):
    ...
    def __len__(self):
        return len(self.rays_o)  # HACK (wjh)
...
@register("clothes-semantic-datamodule")

...
class RandomCameraDataModuleConfig:
    ...
    proba_body: float = 0.76
    proba_face: float = 0.24  # no f'ing for hands!
    dist_face: float = 1.0
    dist_body: float = 1.1
    dist_hand: float = 1.1
...
class RandomCameraIterableDataset(IterableDataset):
    ...
    def _gen_face_cameras(self):
        # sample elevation angles uniformly with a probability 0.5 (biased towards poles)
        azimuth_deg = torch.rand(self.cfg.batch_size) * (80.0 - (-80.0)) - 80.0
        elevation_deg = torch.rand(self.cfg.batch_size) * (5.0 - (-5.0)) + 30.0
        ...
        # sample distances from a uniform distribution bounded by distance_range
        camera_distances = (
            torch.ones_like(elevation_deg) * self.cfg.dist_face
        )  # camera distances are 1.0
        ...
        # default scene center at origin
        center = self.face_center[None, :].repeat(self.cfg.batch_size, 1).cpu()
        # default camera up direction as +z
        up: Float[Tensor, "B 3"] = torch.as_tensor([0, 0, 1], dtype=torch.float32)[
            None, :
        ].repeat(self.cfg.batch_size, 1)
    ...
    def _gen_lhand_cameras(self):
        # sample elevation angles uniformly with a probability 0.5 (biased towards poles)
        range_azi = 60.0
        azimuth_deg = torch.rand(self.cfg.batch_size) * (
            (90.0 + range_azi) - (90.0 - range_azi)
        ) + (90.0 - range_azi)
        elevation_deg = torch.rand(self.cfg.batch_size) * (5.0 - (-5.0))
        ...
        # sample distances from a uniform distribution bounded by distance_range
        camera_distances = (
            torch.ones_like(elevation_deg) * self.cfg.dist_hand
        )  # camera distances are 1.0
        ...
        # default scene center at origin
        center = self.lhand_center[None, :].repeat(self.cfg.batch_size, 1).cpu()
        # default camera up direction as +z
        up: Float[Tensor, "B 3"] = torch.as_tensor([0, 0, 1], dtype=torch.float32)[
            None, :
        ].repeat(self.cfg.batch_size, 1)
        ...
        for mvp in mvp_mtx:
            # (HACK) hard code the control image drawing.
            control_image = draw_body(self.body_joints, mvp[None, :], [768, 512])
            control_image = draw_hand(self.lhand_joints, mvp[None, :], control_image)
            control_signals.append(
                torch.from_numpy(control_image.transpose(2, 0, 1)).to(mvp.dtype) / 255.0
            )
    ...
    def _gen_rhand_cameras(self):
        # sample elevation angles uniformly with a probability 0.5 (biased towards poles)
        range_azi = 60.0
        azimuth_deg = torch.rand(self.cfg.batch_size) * (
            (270.0 + range_azi) - (270.0 - range_azi)
        ) + (270.0 - range_azi)
        elevation_deg = torch.rand(self.cfg.batch_size) * (5.0 - (-5.0))
        ...
        # default scene center at origin
        center = self.rhand_center[None, :].repeat(self.cfg.batch_size, 1).cpu()
        # default camera up direction as +z
        up: Float[Tensor, "B 3"] = torch.as_tensor([0, 0, 1], dtype=torch.float32)[
            None, :
        ].repeat(self.cfg.batch_size, 1)
        ...
        for mvp in mvp_mtx:
            # (HACK) hard code the control image drawing.
            control_image = draw_body(self.body_joints, mvp[None, :], [768, 512])
            control_image = draw_hand(self.rhand_joints, mvp[None, :], control_image)
            control_signals.append(
                torch.from_numpy(control_image.transpose(2, 0, 1)).to(mvp.dtype) / 255.0
            )
    ...
    def collate(self, batch) -> Dict[str, Any]:
        p = random.random()
        if p < self.cfg.proba_body:
            return self._gen_body_cameras()
        elif p < self.cfg.proba_body + self.cfg.proba_face:
            return self._gen_face_cameras()
        elif p < (self.cfg.proba_body + self.cfg.proba_face + 1.0) / 2.0:
            return self._gen_rhand_cameras()
        else:
            return self._gen_lhand_cameras()
    ...
    def gen_sequences(self):
        ...
        save_path = "./FID_render_cams"

        self.cfg.batch_size = n_body
        body_cams = self._gen_body_cameras()
        self.save_cams(body_cams, save_path)

        # self.cfg.batch_size = n_face
        # face_cams = self._gen_face_cameras()
        # self.save_cams(face_cams, save_path)

        # self.cfg.batch_size = n_lhand
        # lhand_cams = self._gen_lhand_cameras()
        # self.save_cams(lhand_cams, save_path)

        # self.cfg.batch_size = n_rhand
        # rhand_cams = self._gen_rhand_cameras()
        # self.save_cams(rhand_cams, save_path)

        a = 1

    def save_cams(self, cams, path="./"):
        import os

        from PIL import Image

        os.makedirs(path, exist_ok=True)
        semantic = cams["semantic"]
        # save the c2ws
        c2ws = cams["c2w"].cpu().numpy()
        if semantic == "body":
            azis = cams["azimuth"].cpu().numpy()
            np.save(os.path.join(path, "azi_{}.npy".format(semantic)), azis)

        np.save(os.path.join(path, "c2w_{}.npy".format(semantic)), c2ws)

        # save the control images
        for i in range(cams["control_signals"].shape[0]):
            img = cams["control_signals"][i].cpu().numpy().transpose(1, 2, 0)
            img = (img * 255).astype(np.uint8)
            img = Image.fromarray(img)
            img.save(os.path.join(path, "{}_{}.png".format(semantic, "%03d" % i)))
...
class GenImageSequenceDataset(Dataset):
    def __init__(self, cfg: Any, split: str) -> None:
        ...
        if mode == "all":
            ...
            # only in front
            azimuth_deg_face = torch.rand(n_face_views) * (80.0 - (-80.0)) - 80.0
            elevation_deg_face = torch.rand(n_face_views) * (5.0 - (-5.0)) + 30.0
...
@register("joint-semantic-datamodule")

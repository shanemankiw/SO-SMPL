...
class CameraAdjusterCallback(Callback):
    def __init__(self, datamodule, adjust_interval=500):
        self.dm = datamodule
        self.adjust_interval = adjust_interval
        self.ref_length = None

    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        # Adjust the datamodule's properties every 2000 steps.
        if self.ref_length is None:
            cur_mesh = pl_module.geometry.isosurface()
            cur_joints = cur_mesh.extras["joints"]
            cur_length = (cur_joints[..., 2].max() - cur_joints[..., 2].min()).item()
            self.ref_length = cur_length
        elif trainer.global_step % self.adjust_interval == 0:
            cur_mesh = pl_module.geometry.isosurface()
            cur_joints = cur_mesh.extras["joints"]
            cur_length = (cur_joints[..., 2].max() - cur_joints[..., 2].min()).item()

            scale = cur_length / self.ref_length  #
            self.ref_length = cur_length
            self.dm.cfg.camera_distance_range[0] *= scale
            self.dm.cfg.camera_distance_range[1] *= scale
            self.dm.train_dataset.lhand_center = cur_joints[19 : 19 + 21].mean(dim=0)
            self.dm.train_dataset.rhand_center = cur_joints[19 + 21 : 19 + 21 * 2].mean(
                dim=0
            )
            self.dm.train_dataset.face_center = cur_joints[-68:].mean(dim=0)

...
def sdf_density_loss(sdf_ref, density, sdf_thres=0.15):
    """
    sdf_ref: torch.Tensor [N, 1]; outside of the mesh, sdf > 0; inside of the mesh, sdf < 0
    density: torch.Tensor [N, 1];
    """
    # density = (density - torch.min(density)) / (torch.max(density) - torch.min(density))
    outside_weight = torch.where(
        sdf_ref > sdf_thres, torch.exp(sdf_ref + 1e-7), torch.zeros_like(sdf_ref)
    )

    loss = torch.mean(density * outside_weight[..., None])
...
def mask_from_depths(
    clothes_depth: Float[Tensor, "B H W 1"],
    human_depth: Float[Tensor, "B H W 1"],  # base depth
    cheat_mode: bool = False,
) -> Float[Tensor, "B H W 1"]:
    if cheat_mode:
        mask_clothes_over_human = torch.logical_and(
            clothes_depth < human_depth, clothes_depth > 0
        ).float()
    else:
        condition1 = torch.logical_and(clothes_depth < human_depth, clothes_depth > 0)
        condition2 = torch.logical_and(clothes_depth > 0, human_depth == 0)
        mask_clothes_over_human = torch.logical_or(condition1, condition2).float()

    return mask_clothes_over_human


def mask_from_depths_soft(
    clothes_depth: Float[Tensor, "B H W 1"],
    human_depth: Float[Tensor, "B H W 1"],  # base depth
    threshold: float = 0.05,
) -> Float[Tensor, "B H W 1"]:
    human_depth = torch.where(human_depth <= 0, 1e6, human_depth)
    mask = torch.sigmoid((human_depth - clothes_depth) / threshold)

    mask = torch.where(clothes_depth <= 0, torch.zeros_like(mask), mask)
...
def normalize_grid_deformation(
    grid_vertex_offsets: Float[Tensor, "Nv 3"],
    points_range: Tuple[float, float] = (0, 1),
    resolution: int = 128,
) -> Float[Tensor, "Nv 3"]:
    return (
        (points_range[1] - points_range[0])
        / (resolution)  # half tet size is approximately 1 / self.resolution

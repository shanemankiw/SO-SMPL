...
@threestudio.register("smpl-clothes-shoes-system")
class ProlificDreamer(BaseLift3DSystem):
    ...
    def forward(self, batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "semantic" in batch and batch["control_signals"] != None and self.training:
            ...
            # NOTE(wjh) we need to set the semantic to "body" or not?
        render_out = {
            ...
            "shoes_mesh": clothes_out["mesh"].extras["shoes_mesh"],
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.C(self.cfg.loss.lambda_normal_consistency) > 0:
            loss_normal_consistency = out["shoes_mesh"].normal_consistency()
        ...
        if self.C(self.cfg.loss.lambda_laplacian_smoothness) > 0:
            loss_laplacian_smoothness = out["shoes_mesh"].laplacian()
        ...
        if (
            ...
            # Make it a little looser, make sure the distance is right
            # target_displacement = torch.where(
            #     out["clothed_mesh"].extras["clothes_mask"][:, None] > 0.5,
            #     0.03 * torch.ones_like(out["clothed_mesh"].extras["displacement"]),
            #     torch.zeros_like(out["clothed_mesh"].extras["displacement"]),
            # )

            # loss_displacement = torch.norm(
            #     out["clothed_mesh"].extras["displacement"] - target_displacement, dim=-1
            # ).mean()
        ...
        """if self.C(self.cfg.loss.lambda_canny > 0):
            loss_canny = binary_cross_entropy(clothes_rgb_edge,
                                              (clothes_normal_edge>0.5).detach().float())
            self.log("train/loss_canny", loss_canny)
            loss += loss_canny * self.C(self.cfg.loss.lambda_canny)"""

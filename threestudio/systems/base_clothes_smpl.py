...
from threestudio.systems.utils import parse_optimizer_clothes, parse_scheduler
...
class BaseSystem(pl.LightningModule, Updateable, SaverMixin):
    ...
    def configure_optimizers(self):
        optim = parse_optimizer_clothes(self.cfg.optimizer, self)
        ret = {
            "optimizer": optim,
        }
...
class BaseLift3DSystem(BaseSystem):
    ...
    class Config(BaseSystem.Config):
        ...
        geometry_clothes_type: str = ""  # added the geometry solely for clothes
        geometry_clothes: dict = field(default_factory=dict)
        geometry_convert_from: Optional[str] = None
        clothes_geometry_convert_from: Optional[str] = None
        ...
        material_clothes_type: str = ""
        material_clothes: dict = field(default_factory=dict)
        ...
        renderer_clothes_type: str = ""
        renderer_clothes: dict = field(default_factory=dict)
        ...
        num_particles: int = 1
        selected_particle: int = 0

    cfg: Config

    def configure(self) -> None:
        self.material = nn.ModuleList()
        # HACK(wjh) when resume, still initialize from existing model.
        if (
            ...
            # First, the base human geometry
            self.geometry.append(
                threestudio.find(self.cfg.geometry_type).create_from(
                    prev_geometry,
                    self.cfg.geometry,
                    copy_net=True,
                )
            ...
            # read previous materials too
            prev_material_cfg = prev_system_cfg.material
            prev_material = threestudio.find(prev_system_cfg.material_type)(
                prev_material_cfg
            )
            ...
            prev_material.load_state_dict(state_dict, strict=False)
            # restore step-dependent states
            prev_material.do_update_step(epoch, global_step, on_load_weights=True)
            # convert from coarse stage geometry
            prev_material = prev_material.to(get_device())

            # First, the base human geometry
            self.material.append(
                threestudio.find(prev_system_cfg.material_type)(prev_material_cfg)
            )
            self.material[0].load_state_dict(state_dict, strict=True)
            cleanup()
        else:
            self.geometry = nn.ModuleList()
            self.geometry.append(
            ...
            self.material.append(
                threestudio.find(self.cfg.material_type)(self.cfg.material)
            )

        # Secondly, the clothes geometry
        if (
            self.cfg.clothes_geometry_convert_from  # from_coarse must be specified
            and not self.cfg.weights  # not initialized from coarse when weights are specified
            # and not self.resumed  # not initialized from coarse when resumed from checkpoints
        ):
            """
            NOTE(wjh)
            we commented the line #and not self.resumed
            since when resumed, we still want to use the create_from to init geometry
            """
            threestudio.info(
                "Initializing clothes geometry from a given checkpoint ..."
            )
            from threestudio.utils.config import load_config, parse_structured

            prev_cfg = load_config(
                os.path.join(
                    os.path.dirname(self.cfg.clothes_geometry_convert_from),
                    "../configs/parsed.yaml",
                )
            )  # TODO: hard-coded relative path
            prev_clothes_system_cfg: BaseLift3DSystem.Config = parse_structured(
                self.Config, prev_cfg.system
            )
            if prev_clothes_system_cfg.geometry_clothes_type != "":
                prev_clothes_geometry_cfg = prev_clothes_system_cfg.geometry_clothes
                prev_clothes_geometry_cfg.update(self.cfg.geometry_convert_override)
                prev_clothes_geometry = threestudio.find(
                    prev_clothes_system_cfg.geometry_clothes_type
                )(prev_clothes_geometry_cfg)
                state_dict, epoch, global_step = load_module_weights(
                    self.cfg.clothes_geometry_convert_from,
                    module_name="geometry.1",  # hard-coded, from the second geometry
                    map_location="cpu",
                )
            else:
                prev_geometry_cfg = prev_clothes_system_cfg.geometry
                prev_geometry_cfg.update(self.cfg.geometry_convert_override)
                prev_clothes_geometry = threestudio.find(
                    prev_clothes_system_cfg.geometry_type
                )(prev_geometry_cfg)

                state_dict, epoch, global_step = load_module_weights(
                    self.cfg.clothes_geometry_convert_from,
                    module_name="geometry",  # hard-coded, from the second geometry
                    map_location="cpu",
                )

            prev_clothes_geometry.load_state_dict(state_dict, strict=False)
            # restore step-dependent states
            prev_clothes_geometry.do_update_step(
                epoch, global_step, on_load_weights=True
            )
            # convert from coarse stage geometry
            prev_clothes_geometry = prev_clothes_geometry.to(get_device())

            # First, the base human geometry
            self.geometry.append(
                threestudio.find(self.cfg.geometry_clothes_type).create_from(
                    prev_clothes_geometry,
                    self.cfg.geometry_clothes,
                    copy_net=False,
                )
            )
            del prev_clothes_geometry
            cleanup()
        else:
            self.geometry.append(
                threestudio.find(self.cfg.geometry_clothes_type)(
                    self.cfg.geometry_clothes
                )
            )

        self.material.append(
            threestudio.find(self.cfg.material_clothes_type)(self.cfg.material_clothes)
        )
        ...
        self.renderer = threestudio.find(self.cfg.renderer_type)(
            ...
            material=self.material[0],
            background=self.background,
        )

        if self.cfg.renderer_clothes_type == "nerf-volume-clothes-renderer":
            self.renderer_clothes = threestudio.find(self.cfg.renderer_clothes_type)(
                self.cfg.renderer_clothes,
                geometry=self.geometry[1],
                geometry_ref=self.geometry[0],
            ...
            self.renderer_clothes = threestudio.find(self.cfg.renderer_clothes_type)(
                self.cfg.renderer_clothes,
                geometry_base=self.geometry[0],
                geometry_clothes=self.geometry[1],
                material_base=self.material[0],
                material_clothes=self.material[1],
                background=self.background,
            )
    ...
    def on_predict_start(self) -> None:
        self.cfg.exporter["clothes_mode"] = True
        self.exporter: Exporter = threestudio.find(self.cfg.exporter_type)(
            self.cfg.exporter,
            geometry_human=self.geometry[0],
            geometry_clothes=self.geometry[1],
            material_human=self.material[0],
            material_clothes=self.material[1],
            background=self.background,
        )
    ...
    def on_predict_epoch_end(self) -> None:
        ...
        for out in exporter_output:
            ...
            save_func(
                f"it{self.true_global_step}-export-{out.save_name}/{out.save_name}",
                **out.params,
            )

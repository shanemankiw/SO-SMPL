...
def parse_optimizer(config, model):
    if isinstance(model.geometry, nn.ModuleList) and hasattr(config, "params"):
        params = []
        for config_name, args in config.params.items():
            if config_name.startswith("geometry"):
                for i in range(len(model.geometry)):
                    key = "geometry.{}.{}".format(i, config_name.split(".")[-1])
                    params.append(
                        {"params": get_parameters(model, key), "name": key, **args}
                    )
            else:
                params.append(
                    {
                        "params": get_parameters(model, config_name),
                        "name": config_name,
                        **args,
                    }
                )
...
def parse_optimizer_clothes(config, model):
    if isinstance(model.geometry, nn.ModuleList) and hasattr(config, "params"):
        params = []
        for config_name, args in config.params.items():
            if config_name.startswith("geometry"):
                # learning only for geometry_clothes, a.k.a. No. 1
                key = "geometry.1.{}".format(config_name.split(".")[-1])
                params.append(
                    {"params": get_parameters(model, key), "name": key, **args}
                )
            else:

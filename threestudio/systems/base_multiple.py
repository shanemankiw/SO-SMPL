...
class BaseLift3DSystem(BaseSystem):
    ...
    def configure(self) -> None:
        if (
            ...
            for i in range(prev_system_cfg.num_particles):
                prev_geometry.append(
                    threestudio.find(prev_system_cfg.geometry_type)(
                        prev_system_cfg.geometry
                    )
                )
        ...
        # HACK: ugly fix for geometry? still useless
    ...
    def on_predict_start(self) -> None:
        self.exporter = nn.ModuleList()
        for i in range(self.cfg.num_particles):
            self.exporter.append(
                threestudio.find(self.cfg.exporter_type)(
                    self.cfg.exporter,
                    geometry=getattr(self, "geometry_{}".format(i)),
                    material=self.material,
                    background=self.background,
                )
            )
    ...
    def predict_step(self, batch, batch_idx):
        for i in range(self.cfg.num_particles):
            if self.exporter[i].cfg.save_video:
    ...
    def on_predict_epoch_end(self) -> None:
        for i in range(self.cfg.num_particles):
            ...
            exporter_output: List[ExporterOutput] = self.exporter[i]()
            for out in exporter_output:
                ...
                save_func(
                    f"it{self.true_global_step}-export/{out.save_name}_ptk{i}",
                    **out.params,
                )

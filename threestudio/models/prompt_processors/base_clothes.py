...
class PromptProcessorOutput:
    ...
    text_embeddings_clothes: Float[Tensor, "N Nf"]
    uncond_text_embeddings_clothes: Float[Tensor, "N Nf"]
    text_embeddings_vd: Float[Tensor, "Nv N Nf"]
    uncond_text_embeddings_vd: Float[Tensor, "Nv N Nf"]
    text_embeddings_clothes_vd: Float[Tensor, "Nv N Nf"]
    uncond_text_embeddings_clothes_vd: Float[Tensor, "Nv N Nf"]
    text_embeddings_sem: Float[Tensor, "Nv N Nf"]
    uncond_text_embeddings_sem: Float[Tensor, "Nv N Nf"]
    directions: List[DirectionConfig]
    semantics: List[SemanticConfig]
    direction2idx: Dict[str, int]
    semantic2idx: Dict[str, int]
    use_perp_neg: bool
    ...
    def get_text_embeddings(
        ...
        clothes_mode: bool = False,  # determine
        ...
        if clothes_mode:
            if view_dependent_prompting:
                ...
                # Get text embeddings
                text_embeddings = self.text_embeddings_clothes_vd[direction_idx]  # type: ignore
                uncond_text_embeddings = self.uncond_text_embeddings_clothes_vd[direction_idx]  # type: ignore
            else:
                text_embeddings = self.text_embeddings_clothes.expand(batch_size, -1, -1)  # type: ignore
                uncond_text_embeddings = self.uncond_text_embeddings_clothes.expand(  # type: ignore
                    batch_size, -1, -1
                )
        elif semantic == "body":
            if view_dependent_prompting:
            ...
            # Get semantic
            semantic_idx = torch.zeros_like(elevation, dtype=torch.long)
            for s in self.semantics:
                semantic_idx[s.condition(semantic)] = self.semantic2idx[s.name]
            text_embeddings = self.text_embeddings_sem[semantic_idx]
            uncond_text_embeddings = self.uncond_text_embeddings_sem[semantic_idx]
...
class PromptProcessor(BaseObject):
    ...
    class Config(BaseObject.Config):
        ...
        prompt_clothes: str = "a green t-shirt"
    ...
    def configure(self) -> None:
        ...
        if self.cfg.semantic_prompt:
            self.semantics = [
                SemanticConfig(
                    "face",
                    lambda s: f"the face of {s}",
                    lambda s: s,
                    lambda s: s == "face",
                ),
                SemanticConfig(
                    "lhand",
                    lambda s: f"the left hand of {s}, left hand side view",
                    lambda s: s,
                    lambda s: s == "lhand",
                ),
                SemanticConfig(
                    "rhand",
                    lambda s: f"the right hand of {s}, right hand side view",
                    lambda s: s,
                    lambda s: s == "rhand",
                ),
            ]
        ...
        if self.cfg.semantic_prompt:
            self.semantic2idx = {s.name: i for i, s in enumerate(self.semantics)}
        ...
        self.prompt_clothes = self.preprocess_prompt(self.cfg.prompt_clothes)
        # use provided negative prompt
        self.negative_prompt = self.cfg.negative_prompt
        self.negative_prompt_clothes = self.cfg.negative_prompt_clothes
        ...
        if self.cfg.use_prompt_debiasing:
            ...
            prompts_clothes = self.get_debiased_prompt(self.prompt_clothes)

            self.prompts_vd = [
                d.prompt(prompt) for d, prompt in zip(self.directions, prompts)
            ]

            self.prompts_clothes_vd = [
                d.prompt(prompt) for d, prompt in zip(self.directions, prompts_clothes)
            ]
        else:
            self.prompts_vd = [
            ...
            self.prompts_clothes_vd = [
                d.prompt(self.cfg.get(f"prompt_clothes_{d.name}", None) or self.prompt_clothes)  # type: ignore
                for d in self.directions
            ]
            if self.cfg.semantic_prompt:
                self.prompts_sem = [
                    d.prompt(self.cfg.get(f"prompt_{d.name}", None) or self.prompt)  # type: ignore
                    for d in self.semantics
                ]
        ...
        self.negative_prompts_clothes_vd = [
            d.negative_prompt(self.negative_prompt_clothes) for d in self.directions
        ]

        if self.cfg.semantic_prompt:
            self.negative_prompts_sem = [
                s.negative_prompt(self.negative_prompt) for s in self.semantics
            ]
    ...
    def load_text_embeddings(self):
        ...
        self.text_embeddings_clothes = self.load_from_cache(self.prompt_clothes)[
            None, ...
        ]
        self.uncond_text_embeddings_clothes = self.load_from_cache(
            self.negative_prompt_clothes
        )[None, ...]
        ...
        self.text_embeddings_clothes_vd = torch.stack(
            [self.load_from_cache(prompt) for prompt in self.prompts_clothes_vd], dim=0
        )
        self.uncond_text_embeddings_clothes_vd = torch.stack(
            [
                self.load_from_cache(prompt)
                for prompt in self.negative_prompts_clothes_vd
            ],
            dim=0,
        )
        if self.cfg.semantic_prompt:
            self.text_embeddings_sem = torch.stack(
                [self.load_from_cache(prompt) for prompt in self.prompts_sem], dim=0
            )
            self.uncond_text_embeddings_sem = torch.stack(
                [self.load_from_cache(prompt) for prompt in self.negative_prompts_sem],
                dim=0,
            )
    ...
    def __call__(self) -> PromptProcessorOutput:
        if self.cfg.semantic_prompt:
            return PromptProcessorOutput(
                ...
                text_embeddings_clothes=self.text_embeddings_clothes,
                uncond_text_embeddings_clothes=self.uncond_text_embeddings_clothes,
                text_embeddings_sem=self.text_embeddings_sem,
                uncond_text_embeddings_sem=self.uncond_text_embeddings_sem,
                text_embeddings_vd=self.text_embeddings_vd,
                uncond_text_embeddings_vd=self.uncond_text_embeddings_vd,
                text_embeddings_clothes_vd=self.text_embeddings_clothes_vd,
                uncond_text_embeddings_clothes_vd=self.uncond_text_embeddings_clothes_vd,
                directions=self.directions,
                semantics=self.semantics,
                semantic2idx=self.semantic2idx,
            ...
            return PromptProcessorOutput(
                ...
                text_embeddings_clothes=self.text_embeddings_clothes,
                uncond_text_embeddings_clothes=self.uncond_text_embeddings_clothes,
                text_embeddings_sem=None,
                uncond_text_embeddings_sem=None,
                text_embeddings_vd=self.text_embeddings_vd,
                uncond_text_embeddings_vd=self.uncond_text_embeddings_vd,
                text_embeddings_clothes_vd=self.text_embeddings_clothes_vd,
                uncond_text_embeddings_clothes_vd=self.uncond_text_embeddings_clothes_vd,

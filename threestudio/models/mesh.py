...
def compute_vertex_normal_out(v_pos, t_pos_idx):
    i0 = t_pos_idx[:, 0]
    i1 = t_pos_idx[:, 1]
    i2 = t_pos_idx[:, 2]
...
class Mesh:
    ...
    def unwrap_uv(
        ...
        if self._v_tex is None and self._t_tex_idx is None:
            self._v_tex, self._t_tex_idx = self._unwrap_uv(
    ...
    def _laplacian_uniform_uv(self):
        ...
        verts, faces = self._v_tex, self._t_tex_idx
    ...
    @staticmethod
    def sample_v_alb(v_tex, texture_map):
        v_uv_normalized = (v_tex * 2 - 1)[None, None]  # [B, 1, Nv, 2]

        texture_map = texture_map.permute(
            2, 0, 1
        )  # Convert from [H, W, C] to [C, H, W]
        texture_map = texture_map.unsqueeze(0).repeat(v_uv_normalized.size(0), 1, 1, 1)
        ...
        return sampled_texture.permute(0, 2, 3, 1)[
            0, 0
        ]  # Convert from [B, C, 1, Nv] to [B, Nv, C]

    def laplacian_uv(self, coarse=True) -> Float[Tensor, ""]:
        with torch.no_grad():
            L = self._laplacian_uniform_uv()
            if not coarse:
                L_dense = L.to_dense()

        # obtain the albedo of each vertex
        v_tex_jittor = (self.v_tex + torch.randn_like(self.v_tex) * 0.005).clip(
            0, 1
        )  # add random jittor to ensure smoothness, approximately 5 pixels of jittor
        v_alb = self.sample_v_alb(v_tex_jittor, self.extras["albedo"])

        if not coarse:
            loss = L_dense.mm(v_alb)
        else:
            loss = L.mm(v_alb)
    ...
    def laplacian(self, coarse=True) -> Float[Tensor, ""]:
        with torch.no_grad():

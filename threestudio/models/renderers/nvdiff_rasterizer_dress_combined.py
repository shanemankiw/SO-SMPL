...
@threestudio.register("nvdiff-rasterizer-dress-combined")
    ...
    def forward(
        ...
        **kwargs
        ...
        clothes_vpos, clothes_normals, clothes_faces = self.seg_clothes_mesh(
            ...
            clothes_mask=mesh.extras["clothes_mask"],
        )
        ...
        gb_clothes_over_human_mask, _ = self.ctx.interpolate_one(
            mesh.extras["clothes_mask"][:, None], rast, mesh.t_pos_idx
        )
        ...
        if render_rgb:
            ...
            """geo_out_clothes = self.geometry_clothes(positions, output_normal=False)
            ...
            gb_rgb_fg = torch.zeros(batch_size, height, width, 3).to(rgb_base)
            gb_rgb_fg[selector] = (
                rgb_base * (1.0 - gb_clothes_over_human_mask[selector])
                + rgb_clothes * gb_clothes_over_human_mask[selector]
            )"""

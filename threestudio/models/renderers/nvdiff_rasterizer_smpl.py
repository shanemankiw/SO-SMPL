...
@threestudio.register("nvdiff-rasterizer-smpl")
        ...
        **kwargs
        ...
        if render_rgb:
            ...
            if self.cfg.noise_std > 0.0:
                ...
                geo_out_seperate_noise = self.geometry(
                    positions_noise, output_normal=False
                )
                albedo_seperate_noise = self.material(
                    viewdirs=gb_viewdirs[selector],
                    positions=positions_noise,
                    light_positions=gb_light_positions[selector],
                    shading_normal=gb_normal[selector],
                    shading="albedo",
                    **geo_out_seperate_noise
                )
                out.update(
                    {
                        "albedo_smooth_loss": F.mse_loss(
                            albedo_seperate, albedo_seperate_noise
                        )
                    }
                )

...
@threestudio.register("nvdiff-rasterizer-smplclothes-uv")
class NVDiffRasterizer(Rasterizer):
    ...
    def forward(
        ...
        **kwargs
        ...
        with torch.no_grad():
            mesh_base = self.geometry_base.isosurface()
        ...
        # obtain the uv values of the base human
        texc, texc_db = self.ctx.interpolate(
            mesh_base._v_tex[None, ...],
            rast,
            mesh_base._t_tex_idx,
            rast_db=rast_db,
            diff_attrs="all",
        )

        albedo = self.ctx.texture(
            mesh_base.extras["albedo"].unsqueeze(0),
            texc,
            uv_da=texc_db,

...
@threestudio.register("controlnet-vsd-smpl-guidance")
class ControlVSDGuidance(BaseModule):
    ...
    def configure(self) -> None:
        ...
        pipe = StableDiffusionControlNetPipeline.from_pretrained(
            ...
            # os.path.join(self.cfg.pretrained_model_name_or_path),
        ...
        if (
            ...
            pipe_lora = StableDiffusionControlNetPipeline.from_pretrained(
                self.cfg.pretrained_model_name_or_path_lora,
                # os.path.join(local_root, self.cfg.pretrained_model_name_or_path_lora),
                controlnet=self.controlnet,
                **pipe_lora_kwargs,
            ).to(self.device)
            del pipe_lora.vae
    ...
    def train_lora(
        self,
        latents: Float[Tensor, "B 4 64 64"],
        control_signals: Float[Tensor, "B 4 512 512"],
        text_embeddings: Float[Tensor, "BB 77 768"],
        camera_condition: Float[Tensor, "B 4 4"],
        controlnet_conditioning_scale: float = 1.0,
    ):
        ...
        down_block_res_samples, mid_block_res_sample = self.controlnet(
            ...
            t,
            encoder_hidden_states=text_embeddings,
            controlnet_cond=control_signals,  # control_image
            conditioning_scale=controlnet_conditioning_scale,
            guess_mode=False,
            return_dict=False,
        )
    ...
    def get_latents(
        ...
        if rgb_as_latents:
            latents = F.interpolate(
                rgb_BCHW, (96, 64), mode="bilinear", align_corners=False
            )  # (96, 64)
        else:
            rgb_BCHW_512 = F.interpolate(
                rgb_BCHW, (768, 512), mode="bilinear", align_corners=False
            )  # (768, 512) is the original
        ...
        **kwargs,
        ...
        if use_lora:
            ...
            loss_lora = self.train_lora(
                latents, control_signals, text_embeddings, camera_condition
            )

...
@threestudio.register("mesh-exporter-uv")
class MeshExporter(Exporter):
    ...
    def __call__(self) -> List[ExporterOutput]:
        if self.cfg.clothes_mode:
            mesh: Mesh = self.geometry.isosurface(clothes_only=True)
        else:
    ...
    def export_obj_with_mtl(self, mesh: Mesh) -> List[ExporterOutput]:
        ...
        if self.cfg.save_texture:
            ...
            # rasterize
            rast, rast_db = self.ctx.rasterize_one(
            ...
            albedo_comb = self.geometry.forward_uv(
                points=gb_pos[None, ...], albedo_features=albedo
            )  # albedo

            # Sample out textures from MLP
            mat_out = self.material.export(features=albedo_comb)

            threestudio.info(
                "Perform UV padding on texture maps to avoid seams, may take a while ..."
            )
            # if "normal" in albedo_comb:
            #     params["map_Bump"] = uv_padding(albedo_comb["normal"])

            if "albedo" in mat_out:
                params["map_Kd"] = uv_padding(mat_out["albedo"][0])
            else:
                threestudio.warn(

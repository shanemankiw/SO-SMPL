...
class ImplicitSDF(BaseImplicitGeometry):
    ...
    def initialize_shape(self) -> None:
        ...
        for _ in tqdm(
            ...
            sdf_pred, sdf_grad = self.forward_sdf(points_rand, no_mask=True)
            loss_eikonal = (
                (torch.linalg.norm(sdf_grad, ord=2, dim=-1) - 1.0) ** 2
            ).mean()
            # sdf_pred = self.forward_sdf(points_rand)
            if _ > 1500:
                loss = F.mse_loss(sdf_pred, sdf_gt) + loss_eikonal * 0.01
            else:
    ...
    def forward_sdf(
        self, points: Float[Tensor, "*N Di"], no_mask=False
    ) -> Float[Tensor, "*N 1"]:
        points_unscaled = points
        points = contract_to_unisphere(points_unscaled, self.bbox, self.unbounded)
        if self.cfg.pos_encoding_config["otype"] == "HashGrid":
            ...
            sdf = self.sdf_network(
                self.encoding(points.reshape(-1, self.cfg.n_input_dims), no_mask)
                # self.encoding(points.reshape(-1, self.cfg.n_input_dims))
            ).reshape(*points.shape[:-1], 1)
        sdf = self.get_shifted_sdf(points_unscaled, sdf)

...
@threestudio.register("smpl-plus")
class SMPLPlus(BaseExplicitGeometry):
    ...
    def export_motions_aist(
        ...
        motion_name = "gKR_sBM_cAll_d28_mKR0_ch05.pkl"
    ...
    def export_motions(self, pc2_path="checkouts/motion/motion_seq.pc2", interval=1):
        ...
        motion_name = "humanml3d_026_smpl_params.npy"  # "humanml3d_021_smpl_params.npy"
        import os
        ...
        root_trans_seq = torch.matmul(root_trans_seq, x_rotation_180)
    ...
    def export_motions_amass(
        ...
        for i, pose in enumerate(poses):
            ...
            v_cano_dense_disp = self._apply_transformations(v_cano_dense_disp)
            v_cano_dense_disp = v_cano_dense_disp[..., [1, 2, 0]]
    ...
    def isosurface(self) -> Mesh:
        ...
        v_dense = subdivide_inorder(
            smpl_out.vertices[0], self.faces[SMPLXSeg.remesh_mask], self.unique
        ).squeeze(0)
        transformed_joints = self._apply_transformations(smpl_out.joints.squeeze())

        mesh = Mesh(
            v_pos=v_dense.contiguous(),  # not contiguous, why?
            t_pos_idx=self.dense_faces,
            # extras
        ...
        v_nrm = mesh._compute_vertex_normal()
        mesh.v_pos = self._apply_transformations(
            self.displacement * self.vertices_scale[:, None] * v_nrm + v_dense
        ).contiguous()
        mesh.extras["v_dense"] = (
            self.displacement * self.vertices_scale[:, None] * v_nrm + v_dense
        ).contiguous()
        mesh.extras["v_smplx"] = self._apply_transformations(v_dense)

        return mesh

    def isosurface_wbetas(self, inp_betas) -> Mesh:
        if self.cfg.model_type == "smplx":
            smpl_out = self.smpl_model(
                body_pose=self.body_pose,
                expression=self.expression,

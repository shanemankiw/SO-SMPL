...
class ClothesDisplacementFree(nn.Module):
    def __init__(
        self,
        size=(25193, 3),
        initial_offset=0.0,
        perturb_range=0.0,
        clothes_type="pants-short",
    ):
        super(ClothesDisplacementFree, self).__init__()
        self.perturb_range = perturb_range
        self.initial_offset = initial_offset
...
@threestudio.register("smpl-plus-clothes-freedisp")
class SMPLPlusClothes(BaseExplicitGeometry):
    ...
    def configure(self) -> None:
        ...
        self.clothes_displacement = ClothesDisplacementFree(
            size=(25193, 3),
            initial_offset=0.0,
    ...
    def isosurface(self, clothes_only=False) -> Mesh:
        ...
        v_pos_outer = self._apply_transformations(
            disp * (self.clothes_vertices_scale * clothes_mask)[:, None]
            + self.v_pos_base
        )
        clothed_human = Mesh(
    ...
    def isosurface_inputclothes(
        ...
        v_pos_outer = self._apply_transformations(
            input_clothes_disp * (self.clothes_vertices_scale * clothes_mask)[:, None]
            + input_v_pos_base
        )
        v_sample_human = self._apply_transformations(input_v_pos_base)
        v_sample_clothes = self._apply_transformations(
            original_disp * (self.clothes_vertices_scale * clothes_mask)[:, None]
            + self.v_pos_base
        )

...
for i in "${!prompts_upper[@]}"; do
    ...
    python3 launch.py --config configs/smplplus-clothes-nomask.yaml --train \
        ...
        tag="${tags_upper[$i]}_cfg75_alb0k_norm50k_lap50k" \
        use_timestamp=false \
        data.batch_size=1 \
        data.up_bias=0.15 \
    ...
    python3 launch.py --config configs/smplplus-clothes-nomask.yaml --export \
        ...
        exp_root_dir="${exp_root_dir}" \
        seed=2337 \
        resume="${exp_root_dir}/${folder_name}/${tags_upper[$i]}_cfg75_alb0k_norm50k_lap50k/ckpts/last.ckpt" \
        system.exporter_type="mesh-exporter-clothes" \
        tag="${tags_upper[$i]}_exportmesh_woalb" \
        use_timestamp=false \
        data.batch_size=1 \
        data.up_bias=0.15 \
